...
//validar celular
 ...
 if(await validarUsuarioExistente(usuario.cedula)){
alert("La C.I. ingresada ya se encuentra registrada");
return;
}

    const request = await fetch('https://rest-service-gestionusuarios.herokuapp.com/api/usuarios', {
        method: 'POST',
        headers: {
...
async function validarUsuarioExistente(cedula) {


        const response = await fetch('https://rest-service-gestionusuarios.herokuapp.com/api/obtenerUsuario/' + cedula, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
